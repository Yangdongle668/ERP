  ...
  <div class="param-page">
    <el-card class="nav">
      <div
        ...
        :key="m.moduleCode"
        class="nav-item"
        :class="{ active: !keyword && m.moduleCode === current }"
        @click="selectModule(m.moduleCode)"
      >
        <span>{{ m.moduleName }}</span>
        <el-tag size="small" type="info" round>{{ m.count }}</el-tag>
      </div>
      <el-empty v-if="!modules.length" description="暂无参数" :image-size="60" />
    </el-card>
    ...
    <div class="main">
      <el-card class="toolbar">
        ...
        <div class="spacer" />
        <span v-if="changed.length" class="form-tip">已修改 {{ changed.length }} 项未保存</span>
        <el-button v-perm="'system:param:update'" type="primary" :loading="saving" :disabled="!changed.length" @click="save">保存</el-button>
      </el-card>

      <div v-loading="loading">
        <el-card v-for="g in groups" :key="g.name" class="group">
          <div class="group-title">{{ g.name }}</div>
          <div v-for="p in g.items" :key="p.key" class="param-row">
            <div class="label">
              <div class="name">
                {{ p.name }}
                <el-tooltip v-if="modified(p)" :content="`默认值：${display(p, p.defaultValue)}`" placement="top">
                  <el-tag size="small" type="warning" effect="plain">已修改</el-tag>
                </el-tooltip>
              </div>
            ...
            <div class="control">
              ...
              <el-switch v-else-if="p.valueType === 'BOOL'" :model-value="boolOf(p)" active-text="是" inactive-text="否" inline-prompt
                         @update:model-value="setBool(p, $event)" />
        ...
        <el-card v-if="!loading && !groups.length"><el-empty description="没有匹配的参数" /></el-card>
      </div>
    </div>
  </div>
</template>
...
<style scoped>
.param-page { display: flex; gap: 12px; align-items: flex-start; }
.nav { width: 200px; flex-shrink: 0; position: sticky; top: 0; }
.nav :deep(.el-card__body) { padding: 8px 0; }
.nav-item { display: flex; justify-content: space-between; align-items: center; padding: 8px 16px; cursor: pointer; }
.nav-item:hover { background: var(--el-fill-color-light); }
.nav-item.active { color: var(--el-color-primary); background: var(--el-color-primary-light-9); font-weight: 500; }
.main { flex: 1; min-width: 0; }
.nav + .main > .el-card:first-child { margin-top: 0; }
.toolbar :deep(.el-card__body) { display: flex; align-items: center; gap: 8px; }
.search { width: 320px; }
.spacer { flex: 1; }
.param-row { display: flex; gap: 16px; padding: 12px 0; border-bottom: 1px solid var(--el-border-color-lighter); }
.param-row:last-child { border-bottom: none; }
.label { width: 240px; flex-shrink: 0; }
.name { font-weight: 600; display: flex; align-items: center; gap: 6px; }
.key { font-size: 12px; color: var(--el-text-color-placeholder); margin-top: 2px; }
.control { flex: 1; min-width: 0; max-width: 520px; }
.desc { font-size: 12px; color: var(--el-text-color-secondary); margin-top: 4px; }
