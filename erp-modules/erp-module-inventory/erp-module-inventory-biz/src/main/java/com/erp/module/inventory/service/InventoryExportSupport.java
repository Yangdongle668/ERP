 ...
 */
@Component
public class InventoryExportSupport {

    private final ParamApi paramApi;
    private final AsyncTaskApi asyncTaskApi;

    public InventoryExportSupport(ParamApi paramApi, AsyncTaskApi asyncTaskApi) {
    ...
    public <T> void export(HttpServletResponse response, String name, List<ExcelColumn<T>> excelColumns, String columns,
        ...
        // AsyncTaskApi 会把当前用户的安全上下文传给后台线程（字段权限仍然生效）
        Long taskId = asyncTaskApi.submit(AsyncTaskApi.TYPE_EXPORT, "导出" + name, "inventory", ctx -> {
