...
| 下层通知上层 / 同层互相通知 | 发布领域事件（定义在发布方 api 中） | 出货确认发布 `ShipmentConfirmedEvent`，销售回写已出货数量，财务生成应收 |
| 需要在同一事务中完成（数量回写） | `@EventListener`（同事务） | 仓库确认入库发布 `StockInConfirmedEvent`，资材在同一事务中回写到货单已入库数量 |
| 业务单据需要改变库存 | 调用 `InventoryDocApi` 生成仓库单据，由仓管员确认后过账（见 08-仓库 README 第 3 节“单据驱动库存”） | 到货单审核 → `InventoryDocApi.createStockIn(PURCHASE_IN)` |
| 下游需要阻止上游撤销 | 上游在撤销前同步发布 `XxxReversingEvent`，下游监听并在不允许时抛出 `BizException` | 仓库反确认销售出库前，财务检查已开票则阻止 |
| 待办、消息、预警 | 调用 system-api 的 `NotifyApi`（发布平台事件），工作台监听存储 | 品质生成检验单时给检验员创建待办 |
...
| 库存超扣 | 条件更新 `qty >= ?`，排序加锁，乐观锁重试（见 08-仓库/02-库存模型与过账） | 仓库过账引擎 |
| 事件重复投递 | 监听方幂等：以来源单据行建唯一约束（如应收 `uk_source_line`、检验单 `source_line_id`） | 各模块监听器 |
...
| 权限点、字典、参数、审批单据、打印单据、定时任务 | 模块声明 `PermissionDefinition`、`DictDefinition`、`ParamDefinition`、`ApprovalBizDefinition`、`PrintBizDefinition`、`@ErpJob`，系统管理启动时同步（见 01-系统管理 README 第 5 节） |
| 工作台看板卡片、BI 指标 | 模块声明 `DashboardCard`、`MetricDefinition` |
| 引用检查（删除主数据前） | 模块实现 `OrgReferenceChecker`、`MaterialReferenceChecker`、`DictReferenceChecker`、`UomReferenceChecker` 等 |
